...
--     reset to 0 and duelRating reset to 1000 (DUEL_RATING_DEFAULT in state.ts).
--     Everything else (XP, other games' bests, cosmetics, etc.) left untouched.
--   - `playerMeta:*` rows (shared public mirror of the same fields, see PlayerMeta
--     in src/types.ts): same four fields reset.
...
-- 2) Zero out every player's private duel counters and rating.
update public.kv
set value = value || jsonb_build_object('duelWins', 0, 'duelLosses', 0, 'duelDraws', 0, 'duelRating', 1000)
